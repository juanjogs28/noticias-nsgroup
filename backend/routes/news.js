...
const fetch = require("node-fetch"); // si no lo tenés: npm i node-fetch@2
...
router.post("/personalized", async (req, res) => {
  try {
    ...
    const resultsPais = await getSearchResults(subscriber.searchIdPais);
    const resultsSector = await getSearchResults(subscriber.searchIdSector);
